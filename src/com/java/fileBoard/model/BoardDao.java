	...
	public ArrayList<BoardDto> getBoardList(int startRow, int endRow) {
		...
		try {
			...
			while (rs.next()) {
				...
				boardDto.setWriteDate(new Date(rs.getTimestamp("write_date").getTime())); // DB에 sysdate(timestamp) -->
																							// 자바의 Date로 바꾸기
	...
	public int update(BoardDto boardDto, int fileUp) {
		...
		try {
			if (fileUp == 0) { // 파일 수정을 하지 않았을 때..
				...
				value = pstmt.executeUpdate();
			} else { // 파일 수정을 했을 때. OR 파일을 삭제 했을 때.파일 수정을 하지 않았을 경우에는 제목, 내용, 패스워드만 변경해주고 파일 수정이나 삭제
						// 가 이뤄졌을 경우에는 파일이름, 파일크기, 파일경로까지 수정해주어야 한다.
